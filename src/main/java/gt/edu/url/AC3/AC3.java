...
import gt.edu.url.AC3.util.FIFOQueue;

public class AC3 {
    public DomainRestore reduceDomains(CSP2 csp) {
    ...
    public DomainRestore reduceDomains(Variable var, Object value, CSP2 csp, Assignment assignment) {
        DomainRestore result = new DomainRestore();
    ...
    private void reduceDomains(FIFOQueue<Variable> queue, CSP2 csp,
                               DomainRestore info) {
        while (!queue.isEmpty()) {
            Variable var = queue.pop();
            for (Constraint2 constraint : csp.getConstraints(var)) {
    ...
    private boolean revise(Variable xi, Variable xj, Constraint2 constraint,
                           CSP2 csp, DomainRestore info) {
